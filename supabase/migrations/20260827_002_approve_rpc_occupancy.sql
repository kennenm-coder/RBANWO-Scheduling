-- Teach the atomic rForce-approval RPC to populate is_full_day / resource_hours,
-- so appointments minted through it agree with the new whole-day guard. Derived
-- from the existing params — no signature change, callers are unaffected.
